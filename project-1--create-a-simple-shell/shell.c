...
void expand_env(char *argument) {
    // Function to expand environment variables (not implemented in this example)
...
void execute_command(char *arguments[], bool background) {
    ...
    if (pid == 0) {
        ...
        if (!background) {
            ...
            exit(EXIT_FAILURE);  // Terminate child process on execvp failure
        }
    } else if (pid > 0) {
        // Parent process
        if (!background) {
            ...
            // Add handling for process termination based on status (Task 5)
        }
    } else {
        perror("fork");
    }
