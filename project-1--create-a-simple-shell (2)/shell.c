...
void execute_command(char *arguments[], bool background) {
    ...
    if (pid == 0) {
        ...
        exit(EXIT_SUCCESS); // Make sure to exit the child process after execution.
    } else if (pid < 0) {
        perror("fork");
    } else {
        if (!background) {
            ...
            if (WIFEXITED(status) && WEXITSTATUS(status) != 0) {
                fprintf(stderr, "Child process failed, exit status %d\n", WEXITSTATUS(status));
            }
				
        }
				if (!background) {
    				alarm(MAX_EXECUTION_TIME); // Set a timer for process termination
    				if (execvp(arguments[0], arguments) == -1) {
        				perror("execvp");
        				exit(EXIT_FAILURE);
    						}
